...
2. The entry point is reachable by the attacker (check authorized, `message::sender` guards, access control).
3. No existing guard already prevents the attack (`throw_unless`, `if`-revert).
