...
Use this file as the local semantic reference for TON standards and core TON contract lessons when auditing FunC, Tolk, or Tact code. It compresses the high-signal requirements from the TEPs and course material the skill is built around. Upstream source links are tracked in `source-links.md`; audit agents should use this local summary during normal audits for determinism.
...
- External message handlers are dangerous by default.
